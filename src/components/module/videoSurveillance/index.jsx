...
const VideoSurveillance = (props) => {
  ...
  //递归添加展示所有的面
  const add_Polygon = (index,data,map3d,neendObj,callback )=>{
    if(!(JSON.stringify(data[index].position) === "{}")){
      Model.createPolygon(map3d, data[index], data[index].position.points.slice(0, 16), ((msg) => {
        neendObj.push(JSON.parse(msg))
        if(++index<data.length){
          add_Polygon(index,data,map3d,neendObj,callback)
        }else{
          message.success("可视区域加载完毕！")
          callback(neendObj)
        }
      }))
    }else{
      ...
      case 3:
        ...
        if (isPolygon) {
          ...
          cameraList_S({ device_code: "" }).then(res => {
            if (res.msg === "success") {
              add_Polygon(0,res.data,mp_light,[],((polygons)=>{
                setapj(polygons)
              }))
            }
          })
